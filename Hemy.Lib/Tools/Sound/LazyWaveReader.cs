...
// /// </summary>
// public unsafe sealed class LazyWaveReader2 : IDisposable
// {
...
//     /// <summary> . </summary>
//     public LazyWaveReader2(string filename)
//     {
//         // fs = new System.IO.FileStream(filename, System.IO.FileMode.Open, System.IO.FileAccess.Read);
//         // _filename = filename;
//         _wavData = Hemy.Lib.Core.Memory.Memory.New<WavData>();
//         /*Hemy.Lib.Core.IO.BinaryFileReader reader = new();
//                     Open_File(  BinaryFileHeader HAndle , byte* filename, )
//             var err= read_Header_Size( BinaryFileHeader HAndle, Size , byte* bufferHeaderRead , overlapped (progress ));
//             position =0
//                     var resultOneCar = Byte() =>  bufferHeaderRead[position++]; //test position != endofheader 
//                      UInt() => (uint)((Byte() + (Byte() << 8)) + ((Byte() + (Byte() << 8)) << 16))
//                      float read float = read UInt = *(* (&uint)) ;
//         */
//     }
...
//     {
//         if (UInt() != WAVE_RIFF_LE) return 1;

//         FileSize = UInt();

//         if (UInt() != WAVE_WAVE_LE) return 1;

//         if (UInt() != WAVE_FMT_LE) return 1;
...
//         if (UInt() != WAVE_DATA_LE) return 1;

//         DataSize = UInt() - WavHeaderSize;
//         Octetparseconde = Frequence * (uint)BytePerBloc;

//         return DataSize <= 44 ? 1 : 0 ;
//     }
...
/// Simple class lecture fichier wav
/// </summary>
public sealed class LazyWaveReader : IDisposable
{

    public static void TestNEwReader(string filename)
    => Hemy.Lib.Core.Platform.Windows.IO.IoFileRWImpl.TestReadBinaryFile(filename);
    ...
    private readonly System.IO.FileStream fs;
    /// <summary> . </summary>
    public LazyWaveReader(string filename)
    {
        fs = new System.IO.FileStream(filename, System.IO.FileMode.Open, System.IO.FileAccess.Read);
        _filename = filename;
    }
